...
CosetInj: (h: Type) -> ((+) : h -> h -> h) -> (pf1: IsGroup h (+)) ->
          (g: Type) -> ((*) : g -> g -> g) -> (pf2: IsGroup g (*)) ->
          (sbgrp: Subgroup h (+) pf1 g (*) pf2) ->
          (trav: Type) -> (f: trav -> g) -> Type
CosetInj  h (+) pf1 g (*) pf2 sbgrp trav f =
  ((x: trav) -> (y: trav) -> (p: h ** (f x) = (f y)*(incl p)) -> (x = y)) where
    incl = (fst sbgrp)


--The type of proofs that a type trav traverses every coset of a given group g wrt a subgroup h
CosetAll: (h: Type) -> ((+) : h -> h -> h) -> (pf1: IsGroup h (+)) ->
          (g: Type) -> ((*) : g -> g -> g) -> (pf2: IsGroup g (*)) ->
          (sbgrp: Subgroup h (+) pf1 g (*) pf2) ->
          (trav: Type) -> (f: trav -> g) -> Type
CosetAll h (+) pf1 g (*) pf2 sbgrp trav f =
  ((a: g) -> (p: h ** (t: trav ** ((f t)*(incl p) = a)))) where
    incl = (fst sbgrp)


-- Traversal type - the type of proofs that a given type traverses each coset exactly once
IsTraversal: (h: Type) -> ((+) : h -> h -> h) -> (pf1: IsGroup h (+)) ->
            (g: Type) -> ((*) : g -> g -> g) -> (pf2: IsGroup g (*)) ->
            (sbgrp: Subgroup h (+) pf1 g (*) pf2) ->
            (trav: Type) -> Type
IsTraversal h (+) pf1 g (*) pf2 sbgrp trav = DPair (trav -> g) (\f => (CosetInj h (+) pf1 g (*) pf2 sbgrp trav f, CosetAll h (+) pf1 g (*) pf2 sbgrp trav f))

MulTrav: (h: Type) -> ((+) : h -> h -> h) -> (pf1: IsGroup h (+)) ->
          (g: Type) -> ((*) : g -> g -> g) -> (pf2: IsGroup g (*)) ->
          (sbgrp: Subgroup h (+) pf1 g (*) pf2) ->
          (trav: Type) -> (IsTraversal h (+) pf1 g (*) pf2 sbgrp trav) ->
          trav -> trav -> trav
...
CorepFInv: (h: Type) -> ((+) : h -> h -> h) -> (pf1: IsGroup h (+)) ->
          (g: Type) -> ((*) : g -> g -> g) -> (pf2: IsGroup g (*)) ->
          (sbgrp: Subgroup h (+) pf1 g (*) pf2) ->
          (trav: Type) -> (pftrav: IsTraversal h (+) pf1 g (*) pf2 sbgrp trav) ->
          (y: trav) ->
          (((fst (snd ((snd (snd pftrav)) ((fst pftrav) y) ) )) ) = y)
...
MulTravDefPf: (h: Type) -> ((+) : h -> h -> h) -> (pf1: IsGroup h (+)) ->
              (g: Type) -> ((*) : g -> g -> g) -> (pf2: IsGroup g (*)) ->
              (sbgrp: Subgroup h (+) pf1 g (*) pf2) ->
              (trav: Type) -> (pftrav: IsTraversal h (+) pf1 g (*) pf2 sbgrp trav) ->
              (y: trav) -> (z: trav) ->
              (MulTrav h (+) pf1 g (*) pf2 sbgrp trav pftrav y z) = ((fst (snd ((snd (snd pftrav)) (((fst pftrav) y)*((fst pftrav) z))) )))
...
MulTravHomPf: (h: Type) -> ((+) : h -> h -> h) -> (pf1: IsGroup h (+)) ->
              (g: Type) -> ((*) : g -> g -> g) -> (pf2: IsGroup g (*)) ->
              (sbgrp: Subgroup h (+) pf1 g (*) pf2) ->
              (trav: Type) -> (pftrav: IsTraversal h (+) pf1 g (*) pf2 sbgrp trav) ->
              (y: g) -> (z: g) ->
              ((fst (snd ((snd (snd pftrav)) (y*z) ))) =
                MulTrav h (+) pf1 g (*) pf2 sbgrp trav pftrav (fst (snd ((snd (snd pftrav)) (y) ))) (fst (snd ((snd (snd pftrav)) (z) ))) )
...
TravIsAssociative: (h: Type) -> ((+) : h -> h -> h) -> (pf1: IsGroup h (+)) ->
                  (g: Type) -> ((*) : g -> g -> g) -> (pf2: IsGroup g (*)) ->
                  (sbgrp: Subgroup h (+) pf1 g (*) pf2) ->
                  (trav: Type) -> (pftrav: IsTraversal h (+) pf1 g (*) pf2 sbgrp trav) ->
                  (Associative trav (MulTrav h (+) pf1 g (*) pf2 sbgrp trav pftrav))
TravIsAssociative h (-) pf1 g (*) pf2 sbgrp trav pftrav a b c = ?l --(Associative grp (*) (f a) (f b) (f c))
