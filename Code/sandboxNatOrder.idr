...
lneqRightTransitive : {a : Nat} -> {b : Nat} -> {c : Nat} -> (LEQ a b) -> (LNEQ b c) -> (LNEQ a c)
lneqRightTransitive {a} {b} {c} leftLEQ rightLNEQ =
	case (leqTransitive leftLEQ (fst rightLEQ)) of

|||Proof that a < b and b <= c implies a < c
lneqLeftTransitive : {a : Nat} -> {b : Nat} -> {c : Nat} -> (LNEQ a b) -> (LEQ b c) -> (LNEQ a c)
lneqRightTransitive {a} {b} {c} ((k ** proofEqLeft), proofNotEq) (l ** proofEqRight) = ((leqTransitive (k ** proofEqLeft) (l ** proofEqRight)), aNotEqc) where
	aNotEqc = nonZeroSumNotEqual aPluskPluslEqc kPluslNotZero where
		kPluslNotZero = \kPluslEqZero => (kNotZero (fst (nonZeroSumNotEqual kPluslEqZero))) where
			kNotZero = \kEqZero => rewrite (sym kEqZero) in
								rewrite (plusCommutative k a) in
								proofEqLeft
		aPluskPluslEqc = rewrite (plusAssociative a k l) in
						trans (cong {f = (\n => n + l)} proofEqLeft) proofEqRight
-}
