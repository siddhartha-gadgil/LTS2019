...
Hom1 : (dom : Type) -> ((*) : dom -> dom -> dom) -> (IsGroup dom (*)) ->
       (cod : Type) -> ((+) : cod -> cod -> cod) -> (IsGroup cod (+)) -> 
       (f : dom -> cod) -> Type

Hom1 dom (*) pfdom cod (+) pfcod f = 
    ((IsIdentity cod (+) (f( fst (Group_id dom (*) pfdom)))) , 
...
Subgroup1: (h: Type) -> ((+) : h -> h -> h) -> (IsGroup h (+)) -> 
           (g: Type) -> ((*) : g -> g -> g) -> (IsGroup g (*)) -> Type
Subgroup1 h (+) pfh g (*) pfg = ( f : (h -> g) ** 
                                (Hom1 h (+) pfh g (*) pfg f , Inj h g f))
...
NSub1: (h: Type) -> ((+) : h -> h -> h) -> (pfh: IsGroup h (+)) -> 
       (g: Type) -> ((*) : g -> g -> g) -> (pfg: IsGroup g (*)) -> 
       (Subgroup1 h (+) pfh g (*) pfg) -> Type
NSub1 h (+) pfh g (*) pfg (f ** pff) = (a : h) -> (b : g) -> (x : h ** (b*(f a)*(inv b) = (f x))) where 
    inv = Inv g (*) pfg     
